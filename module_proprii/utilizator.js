...
//xjxwhotvuuturmqm
    ...
    // 1. O metodă modifica() care primește un obiect cu noile date ale utilizatorului
    modifica(obiectNou) {
        if (!this.username) {
            throw new Error("Utilizatorul nu exista sau nu este selectat.");
        }
        AccesBD.getInstanta(Utilizator.tipConexiune).update({
            ...
            if(err) console.log("Eroare la modificare:", err);
        });
    }

    // 2. O metodă sterge() care șterge din tabel utilizatorul curent
    sterge() {
        if (!this.username) {
        ...
        AccesBD.getInstanta(Utilizator.tipConexiune).delete({
            ...
            if(err) console.log("Eroare la stergere:", err);
        });
    }

    // 3. O metodă statică sincronă
    static cauta(obParam, callback) {
        ...
        AccesBD.getInstanta(Utilizator.tipConexiune).select({
            ...
            if (err) {
                ...
                // Transformam randurile primite de la BD in instante ale clasei Utilizator
                let listaUtilizatori = rezSelect.rows.map(rand => new Utilizator(rand));
                callback(null, listaUtilizatori);
            }
        });
    }

    // 4. O metodă asincronă
    static async cautaAsync(obParam) {
        ...
        try {
            ...
            if (rezSelect && rezSelect.rowCount > 0) {
                return rezSelect.rows.map(rand => new Utilizator(rand));
            }
            return [];
        } catch (e) {
            console.log("Eroare la cautaAsync:", e);
            return [];
        }
    }
